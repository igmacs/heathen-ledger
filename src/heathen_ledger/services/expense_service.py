...
def record_expense(
    ...
    if split_mode == "all":
        ...
        if not participants:
            raise ValidationError("No participants found to split the expense with.")
...
def toggle_split_participant(
    ...
    if target_user_id in current_splits:
        if len(current_splits) <= 1:
            raise ValidationError("Cannot remove the last participant from the split.")
...
def undo_transaction(
    ...
    if clicking_user.id != creator_id:
        ...
        raise PermissionDeniedError(f"Only {creator_name} can undo this transaction.")
