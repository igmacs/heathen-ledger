...
async def pay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    split_msg = (
        f"split specifically between {', '.join(participants)}"
        if participants
        else "split equally among everyone"
    )
...
async def balances(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not bals:
        await context.bot.send_message(
            chat_id=chat_id, text="No active balances in this group."
        )
        return
...
async def clear_ledger(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=chat_id, text="Ledger cleared! All debts have been wiped."
    )
...
def main():
    ...
    application.add_handler(
        MessageHandler(filters.ALL & ~filters.COMMAND, track_active_users)
    )
